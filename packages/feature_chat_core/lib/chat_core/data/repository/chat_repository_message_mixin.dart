...
mixin _ChatMessageRepositoryMixin on ChatRepositoryErrorHandler {
  ...
  Future<ChatMessage> likeChatMessage(String chatMessageId) async {
    return guardChatRequest(() async {
      final message = await _chatMessageDataSource.like(
        chatMessageId: chatMessageId,
      );

      return message.toChatMessage();
    }, fallbackMessage: '좋아요를 반영하지 못했어요. 잠시 후 다시 시도해 주세요.');
  }

  Future<ChatMessage> unlikeChatMessage(String chatMessageId) async {
    return guardChatRequest(() async {
      final message = await _chatMessageDataSource.unlike(
        chatMessageId: chatMessageId,
      );

      return message.toChatMessage();
    }, fallbackMessage: '좋아요를 취소하지 못했어요. 잠시 후 다시 시도해 주세요.');
  }
