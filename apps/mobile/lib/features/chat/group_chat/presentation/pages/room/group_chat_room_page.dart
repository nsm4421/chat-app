...
import 'package:domodachi/core/widgets/app_asset_icon.dart';
...
import 'package:feature_chat_core/chat_core/presentation/bloc/room/chat_message_list_event.dart';
...
import 'package:feature_chat_core/chat_core/presentation/cubit/room/chat_composer_cubit.dart';
import 'package:feature_chat_core/chat_core/presentation/cubit/room/chat_composer_state.dart';
import 'package:feature_group_chat/group_chat/presentation/cubit/room/chat_room_session_cubit.dart';
      ...
      builder: (context, state) {
        ...
        if (state.isNotFound) {
          return _GroupChatRoomUnavailableView(
            ...
            title: '채팅방을 찾을 수 없어요',
            message: state.errorMessage ?? '삭제되었거나 접근할 수 없는 채팅방입니다.',
            actionLabel: '뒤로가기',
...
class _JoinedGroupChatRoomView extends StatelessWidget {
  ...
  Future<bool?> _showRoomInfoBottomSheet(
    BuildContext context,
    ChatRoom room,
            ...
            builder: (context, state) {
              ...
              return Column(
                ...
                children: [
                  ...
                  Text(
                    _roomMetaText(currentRoom, state.members.length),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
          ...
          actions: [
            BlocBuilder<ChatRoomSessionCubit, ChatRoomSessionState>(
              builder: (context, state) => IconButton(
                tooltip: '멤버 보기',
                onPressed: () async {
                  final didDelete = await _showRoomInfoBottomSheet(
                    context,
                    state.room ?? room,
                    state,
...
class _RoomMemberTile extends StatelessWidget {
  ...
  Widget? _buildTrailing(BuildContext context) {
    ...
    return switch (currentRelation.status) {
      RoomMemberFriendStatus.canSendRequest => SizedBox(
        width: 104,
        child: Tooltip(
          message: '친구 요청 보내기',
          child: FilledButton.tonalIcon(
            ...
            icon: const AppAssetIcon(
              kind: AppAssetIconKind.friendAdd,
              size: 18,
              semanticLabel: '친구 요청 보내기',
            ),
            label: const Text('추가'),
          ),
        ),
      ),
      RoomMemberFriendStatus.requestReceived => SizedBox(
        width: 104,
        child: Tooltip(
          message: '친구 요청 수락',
      ...
      RoomMemberFriendStatus.requestSent => SizedBox(
        width: 104,
        child: Align(
      ...
      RoomMemberFriendStatus.friend => SizedBox(
        width: 104,
        child: Align(
          alignment: Alignment.centerRight,
          child: _RelationBadge(
...
class _GroupChatRoomPreviewView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = switch ((isRestricted, isClosed, isFull)) {
      ...
      (_, _, true) => '정원이 가득 찼어요',
      _ => room.isPrivate ? '이 대화를 바로 열 수 있어요' : '이 채팅방에 참여할 수 있어요',
    };

    final message = switch ((isRestricted, isClosed, isFull)) {
      (true, _, _) =>
        room.isPrivate
            ? '이 개인 대화에는 별도 접근 권한이 필요합니다.'
            : '비공개 그룹채팅이라 초대나 별도 접근 권한이 필요합니다.',
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _displayRoomTitle(room, members: state.members),
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
        ),
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _displayRoomTitle(room, members: members),
            style: theme.textTheme.headlineSmall?.copyWith(
          ...
          const SizedBox(height: 16),
          _ParticipantStrip(room: room, members: members),
        ],
      ),
    );
  }
...
class _ParticipantStrip extends StatelessWidget {
  const _ParticipantStrip({required this.room, required this.members});
...
String _displayRoomTitle(
  ...
  final rawTitle = (room.title ?? '').trim();
...
String _defaultRoomDescription(ChatRoom room) {
  if (room.isPrivate) {
    return '친구와 1:1로 대화하는 공간이에요.';
  }

  return '익명 그룹채팅';
}

String _roomMetaText(ChatRoom room, int memberCount) {
  if (room.isPrivate) {
    return '1:1 대화';
  }

  return '$memberCount명 참여 중';
}
