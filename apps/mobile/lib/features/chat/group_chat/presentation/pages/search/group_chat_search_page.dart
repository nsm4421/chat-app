...
class _GroupChatSearchPageState extends State<GroupChatSearchPage> {
  ...
  Future<void> _search() async {
    ...
    try {
      final results = await GetIt.instance<GroupChatUseCases>().searchGroupChatRooms(
        query: query,
      );
      if (!mounted) {
        return;
      ...
      await GetIt.instance<GroupChatUseCases>().saveRecentGroupChatSearchQuery(
        query,
      );
  ...
  Future<void> _loadRecentQueries() async {
    try {
      final queries = await GetIt.instance<GroupChatUseCases>()
  ...
  Future<void> _deleteRecentQuery(String query) async {
    try {
      await GetIt.instance<GroupChatUseCases>().deleteRecentGroupChatSearchQuery(
        query,
      );
      if (!mounted) {
        return;
