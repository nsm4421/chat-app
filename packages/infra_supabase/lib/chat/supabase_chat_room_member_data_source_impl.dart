...
class SupabaseChatRoomMemberDataSourceImpl
  ...
  @override
  Future<bool> isMember({required String chatRoomId, required String userId}) {
    return guardChatRequest(() async {
        ...
        channel!.subscribe((status, [error]) async {
          if (status == RealtimeSubscribeStatus.subscribed) {
            try {
              ...
              controller.addError(ChatDataException(e.toString()));
            }
            return;
          }
          ...
          if (status == RealtimeSubscribeStatus.channelError ||
            ...
            controller.addError(
              ChatDataException(
                error?.toString() ?? '실시간 멤버 목록을 구독하지 못했어요. 잠시 후 다시 시도해 주세요.',
              ),
            );
          }
        });
