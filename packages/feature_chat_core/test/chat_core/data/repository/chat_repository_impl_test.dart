...
import 'package:feature_chat_core/chat_core/data/repository/chat_repository_impl.dart';
...
void main() {
  ...
  group('messages', () {
    test('maps fetched overview messages into domain entities', () async {
      ...
      expect(messages.first.likeCount, 3);
      expect(messages.first.isLikedByMe, true);
    });
    ...
    test('likes and unlikes message through message datasource', () async {
      final likedMessage = await repository.likeChatMessage('message-id');
      final unlikedMessage = await repository.unlikeChatMessage('message-id');

      expect(chatMessageDataSource.likeCallCount, 1);
      expect(chatMessageDataSource.unlikeCallCount, 1);
      expect(likedMessage.likeCount, 4);
      expect(likedMessage.isLikedByMe, true);
      expect(unlikedMessage.likeCount, 2);
      expect(unlikedMessage.isLikedByMe, false);
    });
...
final class _FakeChatMessageDataSource implements ChatMessageDataSource {
  ...
  @override
  Future<ChatMessageOverviewModel> like({required String chatMessageId}) async {
    likeCallCount += 1;
    return ChatMessageOverviewModel(
      id: chatMessageId,
      chatRoomId: 'room-id',
      senderId: 'user-1',
      content: '안녕하세요',
  ...
  @override
  Future<ChatMessageOverviewModel> unlike({
